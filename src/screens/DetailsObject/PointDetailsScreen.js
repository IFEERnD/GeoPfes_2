...
class PointDetailsScreen extends Component {
  ...
  _getData = async () => {
    ...
    let point_id = this.props.navigation.state.params.point_id;
    ...
    if (list_image_td.length > 0) {
      for (let i = 0; i < list_image_td.length; i++) {
        ...
        image_new.point = transformLatLng(
          image_new.location.latitude,
          image_new.location.longitude,
          projection,
        );
        image_new.dateCreate = moment(
          moment(image_new.uri.replace('.jpg', ''), 'YYYYMMDDHHmmSS'),
        ...
        function () {
          if (this.state.points.length > 0) {
            for (let i = 0; i < this.state.points.length; i++) {
              if (this.state.points[i].id === this.state.point_id) {
                let momentObj = moment(
                  this.state.points[i].id,
                  'YYYYMMDDHHmmSS',
                );
                let showDate = moment(momentObj).format('HH:mm MMM DD, YYYY');
                this.setState(
                  {
                    point_details: this.state.points[i],
                    namePoint: this.state.points[i].key,
                    typePoint: this.state.points[i]?.type || '',
                    infoPoint: this.state.points[i]?.info || '',
                    notePoint: this.state.points[i]?.note || '',
                    point_latlong: transformLatLng(
                      this.state.points[i]?.coordinate.latitude,
                      this.state.points[i]?.coordinate.longitude,
                      projection,
                    ),
                    dateCreate: showDate,
                    list_image_old: this.state.points[i]?.list_image || [],
                  },
                  function () {
                    if (this.state.list_image_old.length > 0) {
                      ...
                      for (
                        let j = 0;
                        j < this.state.list_image_old.length;
                        j++
                      ) {
                        images_ = [
                          ...images_,
                          {
                            dateCreate: moment(
                              moment(
                                this.state.list_image_old[j].uri.replace(
                                  '.jpg',
                                  '',
                                ),
                                'YYYYMMDDHHmmSS',
                            ...
                            index: j,
                            location:
                              this.state.list_image_old[j]?.location || 'NaN',
                            point: transformLatLng(
                              this.state.list_image_old[j]?.location?.latitude,
                              this.state.list_image_old[j]?.location?.longitude,
  ...
  _savePoint = async () => {
    ...
    if (this.state.points.length > 0) {
      for (let i = 0; i < this.state.points.length; i++) {
        if (this.state.points[i].id === this.state.point_id) {
          points_new.push({
            coordinate: this.state.points[i].coordinate,
            point: this.state.points[i].point,
            id: this.state.points[i].id,
            list_image: this.state.points[i].list_image,
          ...
          points_new.push(this.state.points[i]);
        }
      }
    }
  ...
  saveImage = async (filePath, folder_name, type) => {
    try {
      ...
      if (this.state.points.length > 0) {
        for (let i = 0; i < this.state.points.length; i++) {
          if (this.state.points[i].id === this.state.point_id) {
            let list_oldimage = this.state.points[i]?.list_image || [];
            list_image_new = [
              ...list_oldimage,
              {
                uri: newImageName,
            ...
            for (let j = 0; j < this.state.list_image_old.length; j++) {
              images_ = [
                ...images_,
                {
                  dateCreate: moment(
                    moment(
                      this.state.list_image_old[j].uri.replace('.jpg', ''),
                      'YYYYMMDDHHmmSS',
                    ),
                  ).format('HH:mm MMM DD, YYYY'),
                  url: `file://${dirHome}/${this.state.folder_name}/${this.state.list_image_old[j].uri}`,
          ...
          .then(image => {
            this.captureRef.handleCapture(
              image.path,
              {
                latitude:
                  transformLatLng(
                    this.state.location_current?.latitude,
                    this.state.location_current?.longitude,
                    this.state.projection,
                  ).lat || 'NaN',
                longitude:
                  transformLatLng(
        ...
        <Card>
          <CardItem>
            <View>
              ...
              <Text>
                {cs_string.date_create}
                {item.dateCreate}
              </Text>
              <Text>
                {cs_string.lat}: {item.point?.lat}
              </Text>
              <Text>
                {cs_string.long}: {item.point?.long}
              </Text>
            </View>
          </CardItem>
        </Card>
          ...
          <CardItem>
            <View>
              <Image
                style={{width: 240, height: 400}}
                source={{
                  uri: `file://${dirHome}/${this.state.folder_name}/${item.uri}`,
                  scale: 1,
                }}
              />
              <View style={{height: 10}} />
      ...
      <View style={{flex: 1}}>
        <Header
          ...
          <Body
            ...
            <Title style={{color: 'white'}}>{cs_string.info_point}</Title>
          </Body>
        ...
        <ScrollView
          ...
          <View style={{flexDirection: 'row'}}>
            <Text
              ...
              {cs_string.local_point}
            </Text>
          </View>
          <View style={{height: 5}} />
          <Text>
            {cs_string.lat}: {this.state.point_latlong?.lat}
          </Text>
          <View style={{height: 5}} />
          <Text>
            {cs_string.long}: {this.state.point_latlong?.long}
          </Text>
          ...
          <View style={{flexDirection: 'row'}}>
            <Text
              ...
              {cs_string.list_photo} ({images.length})
            </Text>
          </View>
        ...
        <View
          ...
          <View style={{flexDirection: 'row'}}>
            <TouchableOpacity
              ...
              <Image
                style={{width: 30, height: 30}}
                source={require('../../images/cancel_black.png')}></Image>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Image
                style={{width: 30, height: 30}}
                source={require('../../images/photo_black.png')}></Image>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Image
                style={{width: 30, height: 30}}
                source={require('../../images/camera_black.png')}></Image>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <Image
                style={{width: 30, height: 30}}
                source={require('../../images/save_black.png')}></Image>
            </TouchableOpacity>
          </View>
        </View>
            ...
            <View style={{flex: 1, backgroundColor: 'white'}}>
              <Header
                ...
                <Body
                  ...
                  <Title style={{color: 'white'}}>{cs_string.list_photo}</Title>
                </Body>
              </Header>

              <FlatList
                data={this.state.list_image_td}
                renderItem={list_image_td => {
                  this._renderItem_list(list_image_td.item);
                }}
                keyExtractor={({item}, index) => index}
                removeClippedSubviews={true}
                initialNumToRender={10}
        ...
        <CaptureImage
          handleCap={path => this.saveImage(path, this.state.folder_name)}
