...
class PolygonDetailsScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            points: [],
    ...
    _getData = async () => {
        ...
        let point_id = this.props.navigation.state.params.polygon_id;
            ...
            this.setState({
                ...
                if (this.state.polygon.length > 0) {
                    for (let i = 0; i < this.state.polygon.length; i++) {
                        if (this.state.polygon[i].id === this.state.point_id) {
                            let momentObj = moment(this.state.polygon[i].id, 'YYYYMMDDHHmmSS');
                            let showDate = moment(momentObj).format('HH:mm MMM DD, YYYY');
                            let areaInKm2 = (this.state.polygon[i].area * 100).toFixed(2);
                            this.setState({
                                point_details: this.state.polygon[i],
                                namePoint: this.state.polygon[i].key,
                                typePoint: this.state.polygon[i]?.type || '',
                                infoPoint: this.state.polygon[i]?.info || '',
                                notePoint: this.state.polygon[i]?.note || '',
                                area: areaInKm2,
                                dateCreate: showDate,
                                list_image_old: this.state.polygon[i]?.list_image || []
                            }, function () {
    ...
    _savePoint = async () => {
        ...
        if (this.state.polygon.length > 0) {
            for (let i = 0; i < this.state.polygon.length; i++) {
                if (this.state.polygon[i].id === this.state.point_id) {
                    points_new.push({
                        coordinates: this.state.polygon[i].coordinates,
                        id: this.state.polygon[i].id,
                        list_image: this.state.polygon[i].list_image,
                        area: this.state.polygon[i].area,
                    ...
                    points_new.push(this.state.polygon[i])
                }
            }
        }
    ...
    saveImage = async (filePath, folder_name, type) => {
        try {
            ...
            if (this.state.polygon.length > 0) {
                for (let i = 0; i < this.state.polygon.length; i++) {
                    if (this.state.polygon[i].id === this.state.point_id) {
                        let list_oldimage = this.state.polygon[i]?.list_image || [];
        ...
        try {
            setTimeout(() => {
                ImagePicker.openCamera({
                    ...
                    console.log(this.state.location_current.latitude)
                ...
                >
                    ...
                    <View style={{ flexDirection: "row" }}>
                        <Text style={{
                            fontSize: 16,
                            fontWeight: 'bold'
                        }}>{cs_string.area_polygon}</Text>
                    </View>
                    <View style={{ height: 5 }} />
                    <Text>{this.state.area} (ha)</Text>
